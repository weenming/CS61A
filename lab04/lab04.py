...
def reverse_recursive(lst):
    ...
    "*** YOUR CODE HERE ***"
    l = len(lst)
    assert l != 0, 'length of lst cannot be zero in recursion'
    half_l = int(l / 2)
    if l == 1:
        return lst[0: 1]
    elif l % 2 == 0:
        return reverse_recursive(lst[half_l:]) + reverse_recursive(lst[0: half_l])
    else:
        return reverse_recursive(lst[half_l + 1:]) + reverse_recursive(lst[half_l: half_l + 1]) + reverse_recursive(lst[0: half_l])
