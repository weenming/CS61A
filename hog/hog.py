...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        this_roll = dice()
        pig_out = pig_out or this_roll == 1
        total_score += this_roll
    if pig_out:
        total_score = 1
    return total_score
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 6
    last_scores = [score0, score1]
    if say == silence:
        say = both(both(say_scores, announce_lead_changes()), both(announce_highest(0, last_scores[0]), announce_highest(1, last_scores[1])))
    while max(scores) < goal:
        this_strategy = strategies[who](scores[who], scores[other(who)])
        this_score = take_turn(this_strategy, scores[other(who)], dice)
        scores[who] += this_score

        # print('DEBUG:', scores)
        if feral_hogs and abs(this_strategy - last_scores[who]) == 2:
            scores[who] += 3
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        this_scores = [score0, score1]
        gain = this_scores[who] - last_score
        if gain > running_high:
            print(gain, 'point(s)! That\'s the biggest gain yet for Player', who)
            return announce_highest(who, this_scores[who], gain)
        return announce_highest(who, this_scores[who], running_high)
    return say
    # END PROBLEM 7
...
def run_experiments():
    ...
    if 1:  # Change to True to test bacon_strategy
